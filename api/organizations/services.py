...
class OrgsServicesManager():
    ...
    def invite_service(user_invited_id, org_id, user_id):
        try:
            user_id = UUID(user_id)
            org_id = UUID(org_id)
            user_invited_id = UUID(user_invited_id)
        except (ValueError, TypeError):
            return error(code="BAD_REQUEST", message="invalid id.")
    ...
    def messages_service(org_id, user_id, before_id, limit):
        try:
            user_id = UUID(user_id)
            org_id = UUID(org_id)
        ...
        if not before_msg:
            return error(code="NOT_FOUND", message="message not found.", status=404)
        
        if not limit:
            return error(code="INVALID_DATA", message="before_id and limit are required.")
        try:
            limit = int(limit)
        except TypeError:
            return error(code="INVALID_DATA", message="limit needs to be int.")
        
        if not 5 <= limit <= 30:
            limit = 30

        messages = Message.query.order_by(Message.created_at.desc()).filter(
        Message.org_id == org_id, or_(Message.created_at < before_msg.created_at, and_(Message.created_at == before_msg.created_at, Message.id!=before_id))
        ).limit(limit+1).options(joinedload(Message.user)).all()
