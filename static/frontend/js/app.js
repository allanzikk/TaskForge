...
const defaultApiBase = isDev
  ? "http://127.0.0.1:5000"
  : "https://api.taskforge.app.br";

const defaultSocketBase = isDev
  ? "http://127.0.0.1:5000"
  : "https://api.taskforge.app.br";
...
async function loadDashboard() {
  ...
  const [userPayload, invitesPayload] = await Promise.all([
    apiRequest(`/users/${encodeURIComponent(userId)}`).catch((e) => {
...
async function loadProfilePage() {
  ...
  const searchPayload = await apiRequest(`/users?username=${encodeURIComponent(username)}`).catch((e) => {
    if (isAuthFailure(e)) throw e; return null;
  });
  ...
  const userPayload = await apiRequest(`/users/${encodeURIComponent(found.id)}`).catch((e) => {
    if (isAuthFailure(e)) throw e; return null;
  });
...
async function handleUserSearch() {
  ...
  await runWithStatus($("#user-search-btn"), async () => {
    const payload = await apiRequest(`/users?username=${encodeURIComponent(query)}`);
...
async function handleSearchUser() {
  ...
  await runWithStatus($("#invite-search-btn"), async () => {
    const payload = await apiRequest(`/users?username=${encodeURIComponent(username)}`);
    const list = Array.isArray(readData(payload)) ? readData(payload) : [];
...
function initChatDrawer() {
  document.getElementById("open-chat-drawer")?.addEventListener("click", openChatDrawer);
  document.getElementById("close-chat-drawer")?.addEventListener("click", closeChatDrawer);
  document.getElementById("close-chat-drawer-2")?.addEventListener("click", closeChatDrawer);
  document.getElementById("chat-backdrop")?.addEventListener("click", closeChatDrawer);
  document.getElementById("chat-back-btn")?.addEventListener("click", showChatOrgList);
...
function openChatDrawer() {
  ...
  document.getElementById("chat-drawer")?.classList.add("is-open");
  document.getElementById("chat-backdrop")?.classList.add("is-open");
  document.getElementById("chat-drawer")?.removeAttribute("aria-hidden");
  renderChatOrgList();

  if (!appState.chat.socket) {
    appState.chat.socket = io(defaultSocketBase, { auth: { token: appState.token } });
    appState.chat.socket.on("error", (data) => toast(data?.message || "Erro no chat.", "error"));
  }

  // Rebind to drawer renderers when drawer is opened
  appState.chat.socket.off("chat_history");
  appState.chat.socket.off("new_message");
  appState.chat.socket.on("chat_history", renderChatHistory);
  appState.chat.socket.on("new_message", appendChatMessage);
}

function closeChatDrawer() {
  document.getElementById("chat-drawer")?.classList.remove("is-open");
  document.getElementById("chat-backdrop")?.classList.remove("is-open");
  document.getElementById("chat-drawer")?.setAttribute("aria-hidden", "true");
}

function renderChatOrgList() {
  const list = document.getElementById("chat-org-list");
  if (!list) return;
  list.innerHTML = "";

  const orgs = appState.organizations.length
  ...
  if (!orgs.length) {
    list.append(emptyMessage("Nenhuma organização encontrada."));
    return;
  }

  orgs.forEach((org) => {
    const btn = document.createElement("button");
    btn.className = "chat-org-item";
    btn.type = "button";

    const avatar = document.createElement("span");
    avatar.className = "chat-org-avatar";
    if (org.image_url) {
      const img = document.createElement("img");
      img.src = org.image_url;
      img.alt = org.name;
    ...
    name.className = "chat-org-name";
    name.textContent = org.name;

    btn.append(avatar, name);
    btn.addEventListener("click", () => openOrgChat(org));
    list.append(btn);
  });
}
...
function openOrgChat(org) {
  ...
  document.getElementById("chat-org-list-view")?.classList.add("is-hidden");
  document.getElementById("chat-messages-view")?.classList.remove("is-hidden");
...
function showChatOrgList() {
  document.getElementById("chat-messages-view")?.classList.add("is-hidden");
  document.getElementById("chat-org-list-view")?.classList.remove("is-hidden");
  appState.chat.currentOrgId = null;
}
...
function sendChatMessage() {
  ...
  if (!content || !appState.chat.currentOrgId || !appState.chat.socket) return;

  appState.chat.socket.emit("send_message", {
    access_token: appState.token,
    org_id: appState.chat.currentOrgId,
    content,
  });
...
function initOrgSideTabs() {
  const tabs = document.querySelectorAll(".org-side-tab");
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      tabs.forEach((t) => t.classList.remove("is-active"));
      tab.classList.add("is-active");
      const target = tab.dataset.tab;
      document.querySelectorAll(".org-side-pane").forEach((pane) => {
        pane.classList.toggle("is-hidden", pane.id !== `tab-${target}`);
      });
      if (target === "chat") initOrgInlineChat();
    });
  });
}

function initOrgInlineChat() {
  const orgId = getQueryParam("id");
  if (!orgId) return;
  ...
  // Always rebind to inline renderers when on org page
  appState.chat.socket.off("chat_history");
  appState.chat.socket.off("new_message");
  appState.chat.socket.on("chat_history", renderOrgChatHistory);
  appState.chat.socket.on("new_message", appendOrgChatMessage);
  ...
  if (sendBtn && !sendBtn.dataset.bound) {
    ...
    sendBtn.addEventListener("click", sendOrgChatMessage);
    input?.addEventListener("keydown", (e) => { if (e.key === "Enter") sendOrgChatMessage(); });
  }
}

function sendOrgChatMessage() {
  const input = document.getElementById("org-chat-input");
  const content = input?.value.trim();
  if (!content || !appState.chat.currentOrgId || !appState.chat.socket) return;
  appState.chat.socket.emit("send_message", {
...
function renderOrgChatHistory(data) {
